...
export const verifyIssue = asyncHandler(async (req, res) => {
  ...
  if (!user.isProfileComplete) {
    throw new ApiError(403, 'Please complete your profile before verifying issues');
  }

  // Find the issue
  const issue = await Issue.findById(issueId).populate('reportedBy', 'username email');
  if (!issue) {
  ...
  if (issue.reportedBy && issue.reportedBy._id.toString() !== userId.toString()) {
    await Notification.create({
      recipient: issue.reportedBy._id,
      type: 'issue_verification',
      title: 'Issue Verified',
      message: `Your issue (${issue.issueId}) has received a public verification (${status === 'correct' ? 'Correct' : 'Incorrect'}) by a community member.`,
...
export const getVerificationStats = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(
    new ApiResponse(200, {
      ...
      totalVerifications: (issue.verifiedCount || 0) + (issue.incorrectCount || 0),
...
export const getVerifierList = asyncHandler(async (req, res) => {
  ...
  const verifications = await CommunityVerification.find({ issue: issueId })
    .populate({
      path: 'verifiedBy',
      select: 'username fullAddress aadhaarNumber mobileNumber isProfileComplete communityDetails',
    })
    .sort({ createdAt: -1 });
    ...
    .map(v => {
      ...
      return {
        ...
        verifier: {
          ...
          area: verifier.fullAddress?.area || verifier.communityDetails?.area || 'N/A',
  ...
  res.status(200).json(
    new ApiResponse(200, {
      verifierList,
      totalCorrect: verifications.filter(v => v.status === 'correct').length,
      totalIncorrect: verifications.filter(v => v.status === 'incorrect').length,
...
export const getUnverifiedIssuesInArea = asyncHandler(async (req, res) => {
  ...
  const verifiedByUser = await CommunityVerification.find({ verifiedBy: userId }).select('issue');
